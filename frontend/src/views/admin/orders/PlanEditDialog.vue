  ...
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          <label class="input-label">{{ t('payment.admin.headcountLimit') }}</label>
          <input v-model.number="planForm.headcount_limit" type="number" min="0" class="input" :placeholder="t('payment.admin.headcountLimitPlaceholder')" />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.headcountLimitHint') }}</p>
        </div>
      </div>
      <div>
...
function buildPlanPayload() {
  ...
  return {
    ...
    headcount_limit: planForm.headcount_limit || 0,
