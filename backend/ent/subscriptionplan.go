...
func (*SubscriptionPlan) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case subscriptionplan.FieldID, subscriptionplan.FieldGroupID, subscriptionplan.FieldValidityDays, subscriptionplan.FieldSortOrder, subscriptionplan.FieldHeadcountLimit:
...
func (_m *SubscriptionPlan) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case subscriptionplan.FieldHeadcountLimit:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field headcount_limit", values[i])
			} else if value.Valid {
